...
class PathAnimationSystem {
   ...
   initUI() {
        const targetDiv = document.getElementById('animateObjbyPath');
        if (!targetDiv) return;
        ...
        wrapper.style.cssText = `
            ...
            font-family: 'Segoe UI', Arial, sans-serif; font-size: 11px; color: #b8b8b8;
        ...
        toolbar.style.cssText = `
            ...
            padding: 4px; background-color: #292929; border-bottom: 1px solid #000;
            min-height: 28px; flex-shrink: 0;
        `;

        // Button Factory
        const createBtn = (id, text, onClick, color = '#444') => {
            const btn = document.createElement('button');
            btn.id = id; 
            btn.textContent = text;
            btn.onclick = onClick;
            btn.style.cssText = `
                background: linear-gradient(to bottom, ${color}, #222);
                border: 1px solid #111; color: #eee;
            ...
            btn.onmouseleave = () => btn.style.background = `linear-gradient(to bottom, ${color}, #222)`;
            return btn;
        };

        // --- BUTTONS (IDs Changed to avoid conflict) ---
        
        const btnDraw = createBtn('draw-path-button', 'Draw Path', () => this.togglePathDrawing());
        const btnEdit = createBtn('edit-path-button', 'Edit Path', () => this.togglePathEditing());
        
        // ✅ CHANGED ID from 'play' to 'path-play-btn' to fix timeline conflict
        const btnPlay = createBtn('path-play-btn', 'Play', () => this.playSelectedAnimation(), '#2a442a'); 
        
        // ✅ CHANGED ID from 'stop' to 'path-stop-btn'
        const btnStop = createBtn('path-stop-btn', 'Stop', () => this.stopAllAnimations(), '#442a2a');
        
        const btnList = createBtn('show-paths-button', 'Paths List', () => {
             const listArea = document.getElementById('paths-list-wrapper');
             if(listArea.style.display === 'none') {
                 listArea.style.display = 'flex';
                 btnList.style.color = '#dfa800'; 
             } else {
                 listArea.style.display = 'none';
                 btnList.style.color = '#eee';
             }
        });
        btnList.style.color = '#dfa800'; 
        ...
        status.id = 'path-status-indicator';
        status.textContent = 'Ready';
        status.style.cssText = `
            margin-left: auto; padding: 0 8px; color: #dfa800; font-family: monospace;
            max-width: 120px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
        `;
        toolbar.appendChild(status);

        const listWrapper = document.createElement('div');
        listWrapper.id = 'paths-list-wrapper';
        ...
        headerRow.style.cssText = `
            display: grid; grid-template-columns: 20px 2fr 1fr 1fr 60px;
            background: #222; border-bottom: 1px solid #000; color: #888; font-weight: 600;
            padding: 2px 0; font-size: 10px; flex-shrink: 0;
        `;
        ['', 'Object', 'Pts', 'Time', 'Actions'].forEach(t => {
        ...
        listContainer.style.cssText = `flex-grow: 1; overflow-y: auto; background: #141414;`;
        ...
        style.textContent = `
            #paths-list-container::-webkit-scrollbar { width: 6px; background: #111; }
            #paths-list-container::-webkit-scrollbar-thumb { background: #333; border-radius: 3px; }
        `;
    ...
    createPathsListModal() {
        ...
        background: #1e1e24; color: #d0d0d8;
        border: 1px solid #333842; border-radius: 5px;
        box-shadow: 0 10px 40px rgba(0,0,0,0.7);
        font-family: 'Segoe UI', sans-serif; font-size: 13px;
        z-index: 2000; display: none;
    `;

        // Header – Houdini style
        const header = document.createElement('div');
        header.style.cssText = `
        display: flex; justify-content: space-between; align-items: center;
        padding: 10px 14px; background: #252b32; border-bottom: 1px solid #333842;
    `;

        const title = document.createElement('h3');
        title.textContent = 'Animation Paths';
        title.style.cssText = 'margin:0; font-size:15px; font-weight:500; color:#e0e0ff;';
    ...
    `;
        close.onmouseenter = () => close.style.color = '#ff6666';
        close.onmouseleave = () => close.style.color = '#aaa';
    ...
    updatePathsList() {
        ...
        if (this.paths.size === 0) {
            ...
            empty.textContent = 'No paths created. Select object & Click Draw.';
            empty.style.cssText = 'padding: 20px; text-align: center; color: #444; font-style: italic;';
        ...
        this.paths.forEach((pathData, object) => {
            ...
            row.style.cssText = `
                display: grid;
                grid-template-columns: 24px 2fr 1fr 1fr 60px;
                background-color: ${isOdd ? '#1d1d1d' : '#191919'};
            ...
            row.onmouseenter = () => row.style.backgroundColor = '#303030';
            row.onmouseleave = () => row.style.backgroundColor = isOdd ? '#1d1d1d' : '#191919';

            // 1. Color Swatch
            const colorCell = document.createElement('div');
            colorCell.style.cssText = `
                width: 12px; height: 12px;
                background: #${pathData.color.toString(16).padStart(6, '0')};
                margin: 0 auto;
            ...
            nameCell.textContent = object.name || `Object_${object.uuid.substring(0, 4)}`;
            nameCell.style.cssText = 'padding-left: 6px; border-right: 1px solid #111; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; height: 100%; line-height: 22px;';
            if(object === this.selectedObject) nameCell.style.color = '#dfa800';

            // 3. Points
            const ptsCell = document.createElement('div');
            ptsCell.textContent = pathData.points.length;
            ptsCell.style.cssText = 'padding-left: 6px; color: #8ee; border-right: 1px solid #111; height: 100%; line-height: 22px;';

            // 4. Duration
            const durCell = document.createElement('div');
            durCell.textContent = (pathData.duration || 5.0).toFixed(1) + 's';
            durCell.style.cssText = 'padding-left: 6px; color: #9f9; border-right: 1px solid #111; height: 100%; line-height: 22px;';

            // 5. Actions
            const actionsCell = document.createElement('div');
            actionsCell.style.cssText = 'display: flex; justify-content: space-around; align-items: center; padding: 0 4px;';

            const createIconBtn = (char, color, cb) => {
                const b = document.createElement('span');
                b.innerHTML = char;
                b.style.cssText = `cursor: pointer; color: ${color}; font-size: 12px; font-weight: bold; opacity: 0.7;`;
            ...
            actionsCell.appendChild(createIconBtn('▶', '#5f5', () => this.playAnimation(object)));
            actionsCell.appendChild(createIconBtn('✎', '#5af', () => this.editPath(object)));
            actionsCell.appendChild(createIconBtn('×', '#f55', () => { this.deletePath(object); this.updatePathsList(); }));
            ...
            row.onclick = () => {
                ...
                this.updatePathsList(); // Re-render to show orange highlight
            };
    ...
    finishPath() {
        ...
        this.updatePathVisualization(this.selectedObject, pathData);
        this.currentPoints = [];
        document.getElementById("path-status-indicator").textContent =
            `Path created for ${this.selectedObject.name || "Selected Object"}`;
        this.updatePathsList();
    }
    ...
    togglePathEditing() {
        ...
        if (this.pathEditMode) {
            ...
            button.style.background = "#28a745";
            document.getElementById("path-status-indicator").textContent =
                `Editing path for ${this.selectedObject.name || "Selected Object"}`;
    ...
    createControlPoints(points) {
        ...
        const geometry = new THREE.SphereGeometry(
            this.camera.position.length() * 0.05,
            16,
            16,
        );
    ...
    deletePath(object) {
        ...
        document.getElementById("path-status-indicator").textContent =
            `Path deleted for ${object.name || "Selected Object"}`;
        this.updatePathsList();
    }
    ...
    hookedPlayAnimation(object) {
        console.error(
            "Path system not correctly initialized! Missing baking hook.",
        );
    }
    ...
    update(deltaTime) {
        ...
        this.animatingObjects.forEach((animData, object) => {
            ...
            const objectKeyframes =
                typeof keyframes !== "undefined" ? keyframes.get(object.uuid) : null;
...
function initializePathAnimationSystem(scene, camera, renderer) {
    ...
    if (
        ...
        typeof THREE.CatmullRomCurve3 === "undefined"
    ) {
        console.error(
            "PathAnimationSystem: THREE.js components required are missing.",
        );
        return;
    }
    ...
    window.pathAnimator.hookedPlayAnimation = function (object) {
        if (
            typeof bakePathToKeyframes !== "function" ||
            typeof playAnimation !== "function"
        ) {
            console.error(
                "Path System: Timeline integration functions (bakePathToKeyframes, playAnimation) are missing. Ensure timeline.js is loaded and initialized.",
            );
            return;
        }
        ...
        document.getElementById("path-status-indicator").textContent =
            `Path baked and playing for ${object.name}`;
...
/*document.addEventListener("DOMContentLoaded", function () {
    ...
    const showIconPath =
        "M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm-1-13h2v6h-2z";
    const hideIconPath =
        "M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm1-3h-2v-6h2z";
...
document.addEventListener("DOMContentLoaded", function () {
    const toggleButton = document.getElementById("toggleContainerButton");
    const animationContainer = document.getElementById("animateObjbyPath");
    const iconPath = document.getElementById("icon-path");

    if (!toggleButton || !animationContainer) {
        return;
    }
    ...
    // 1. Force Hidden Initially
    animationContainer.style.display = "none";
    if(iconPath) iconPath.setAttribute("d", hideIconPath);
    let isVisible = false;
    ...
    toggleButton.addEventListener("click", function () {
        ...
        if (isVisible) {
            ...
            if(iconPath) iconPath.setAttribute("d", showIconPath);
            
            // Trigger refresh in case data changed while hidden
            if(window.pathAnimator) window.pathAnimator.updatePathsList();
        } else {
            // Hide Container
            animationContainer.style.display = "none";
            if(iconPath) iconPath.setAttribute("d", hideIconPath);
...
function bakePathToKeyframes(
    ...
    console.log(
        `✅ Baked path animation for ${object.name || "object"} into ${Object.keys(objectKeyframes).length} keyframes`,
    );
}
