...
class PathAnimationSystem {
   ...
   playSelectedAnimation() {
    ...
    // Sync timeline to beginning or keep its state
    currentTime = 0;
    updatePlayhead();
    playAnimation(); // Call the global play function from the timeline
}
    ...
    resumeAllAnimations() {
        this.animatingObjects.forEach((data, object) => {
            // Already handled in update using currentTime
            // You can optionally reset other flags here
        });
    }
    ...
    this.animatingObjects.forEach((animData, object) => {
        ...
        // Natural timing: progress = time / duration
        let progress = currentTime / animData.duration;
