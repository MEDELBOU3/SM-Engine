...
class GlobalViewportManager {
  ...
  initRenderer() {
    ...
    if (!this.renderer) {
      console.error("[GlobalViewportManager] Shared renderer not available.");
      return;
    }
  ...
  registerState(id, config) {
    if (!config.scene || !config.camera || !config.targetElementId) {
      console.error(
        `ViewportManager: Missing required config for state '${id}'`,
      );
      return;
    }
  ...
  async transitionTo(nextStateId) {
    ...
    if (!nextState || !nextState.targetContainer) {
      console.warn(
        `ViewportManager: State '${nextStateId}' not found or missing container.`,
      );
      return;
    }
  ...
  startRenderLoop() {
    const animate = () => {
      ...
      if (this.isTransitioning && this.fadeOverlay.style.opacity === "1")
        return;
