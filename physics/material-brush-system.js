...
function update3DBrushPreviewOnMouseMove(event, forceVisible = false) {
    ...
    /*if (selectedTool !== TOOLS.MATERIAL_PAINT && selectedTool !== TOOLS.TEXTURE_PAINT ) {
    if (!forceVisible) brushPreviewMesh.visible = false; // Hide if not the right tool, unless forced
        // You might have a generic part of brushPreviewMesh (like an outer ring)
        // that you want to show for sculpt tools, handled by an else if block.
        //But if this function is *only* for material paint advanced preview:
