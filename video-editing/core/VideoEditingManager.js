 ...
 * Phase 3: runtime composition is derived from VideoProjectState clips.
 */
...
class SMVideoCanvasController {
    ...
    init() {
        ...
        // Blender Top-Right Nav Overlay (Zoom & Pan gizmo)
    ...
    refreshOverlay() {
        ...
        if (
            ...
            /*
             * Audio clips are timeline/audio objects, not visual composition
             * layers. They must never show transform handles in the canvas.
             */
...
class SMVideoEditingManager {
    constructor() {
        ...
        /*
         * PHASE 3:
         * Timeline clips are persistent composition data.
         * this.items is now only a runtime cache generated from those clips.
         */
        this.project =
            window.ensureVideoProjectState?.() ||
            window.videoProject ||
            null;

        this.itemAdapter =
            window.VideoCanvasItemAdapter
                ? new window.VideoCanvasItemAdapter(
                    this.project,
                    this
                )
                : null;

        this.compositionRuntime =
            window.VideoCompositionRuntime
                ? new window.VideoCompositionRuntime(
                    this.project,
                    this,
                    this.itemAdapter
                )
        ...
        this.resolution = {
            w:
                Number(
                    this.project?.settings?.resolution?.w ||
                    1280
                ),
            h:
                Number(
                    this.project?.settings?.resolution?.h ||
                    720
                )
        };
        ...
        this.masterGain =
            Math.max(
                0,
                Number(
                    this.project?.audio?.master?.gain ??
                    1
                )
            );
    ...
    enter() {
        ...
        const seq = window.ensureSequencerManager(
            this,
            this.project
        );
    ...
    suspendWorkspace() {
        ...
        document.body.classList.add(
            'video-edit-workspace-suspended'
        );
        ...
        if (sequencerRoot) {
            sequencerRoot.style.display = 'none';
        }

        const splitter =
            document.querySelector(
                '#sequencer-root .sequencer-splitter, .sequencer-splitter'
            );
        ...
        const legacyTimeline =
            document.getElementById('video-timeline-panel');

        if (legacyTimeline) {
            legacyTimeline.style.display = 'none';
        }

        document.getElementById('video-editor-toolbar')
            ?.style
            .setProperty(
                'display',
                'flex',
    ...
    _drawMediaItem(item) {
        ...
        /*
         * FUSION LIVE COMPOSITOR
         * ----------------------
         * VideoFusionRuntimeBridge evaluates the selected clip graph during
         * videoCompositionBeforeDraw and attaches the result only for this
         * draw call.
         *
         * The Fusion canvas replaces the raw MediaIn pixels, while the normal
         * clip transform/opacity/blend stack outside this method still works.
         */
        const fusionSource =
            item._fusionVisualSource;

        if (
            fusionSource &&
            fusionSource.width > 0 &&
            fusionSource.height > 0
        ) {
            this._drawMediaContain(
                fusionSource,
                fusionSource.width,
                fusionSource.height,
                x,
        ...
        if (item.mediaType === 'image') {
             ...
             * AUDIO IS NON-VISUAL.
             *
             * Keep the HTMLAudioElement synchronized with the timeline, but
             * NEVER paint a waveform/placeholder into the composition canvas.
             * Waveforms belong only in the Sequencer / Audio Studio UI.
             */
            ...
            /*
             * Unknown media types are not automatically treated as audio
             * visuals. A media item must explicitly provide a visual renderer.
             */
            return;
        }
    }
    ...
    _applyMediaVolume(media, item) {
        ...
        /*
         * Audio Studio:
         * after this element is routed into Web Audio, clip/track/master
         * attenuation is handled by AudioNode gains. Keep the media element
         * itself at unity to avoid applying volume twice.
         */
        if (
            window.audioStudioManager
                ?.isMediaRouted?.(media)
        ) {
            try {
        ...
        // Native fallback before Audio Studio routing.
        const master =
            Math.max(
                0,
    ...
    deleteItem(item) {
        ...
        /*
         * Persistent deletion happens in Sequencer/VideoProjectState.
         * CompositionRuntime notices the deleted clip and releases native
         * video/audio elements automatically.
         */
        if (seq?.removeClipsByItem) {
            seq.removeClipsByItem(
                runtimeId
            );
        } else {
            const clips =
    ...
    _syncHierarchy() {
        ...
        /*
         * CRITICAL:
         * The Media Pool belongs exclusively to MediaPoolManager.
         *
         * Older SM Engine code used:
         *
         *   document.getElementById('media-pool-grid')
         *
         * as a fallback hierarchy container. That erased imported Media Pool
         * thumbnails every time the compositor synchronized its hierarchy and
         * replaced them with "No video canvas items".
         *
         * Never write to #media-pool-grid from VideoEditingManager.
         */
        ...
        /*
         * Optional dedicated compositor-items host.
         * If the Hierarchy UI wants a special video-items section, create:
         *
         *   <div id="video-canvas-items-list"></div>
         *
         * Otherwise HierarchyManager remains the authority and we do not
         * mutate any generic hierarchy container here.
         */
        const container =
            document.getElementById(
                'video-canvas-items-list'
            ) ||
            document.querySelector(
                '[data-video-canvas-items-list]'
            );
        ...
        if (container) {
            ...
            if (!this.items.length) {
                ...
                this.items.forEach(item => {
                    ...
                    row.innerHTML = `
                        <span class="hm-row-video-main">
                            <i class="fas ${icon}"></i>
                            <strong></strong>
                        </span>

                        <span class="hm-row-video-remove">
        ...
        try {
            window.dispatchEvent(
                new CustomEvent(
                    'videoCompositionHierarchyChanged',
                    {
                        detail: {
                            items:
                                this.items,
        ...
        /*
         * Re-render Media Pool only through its owner. This is cheap and also
         * recovers the UI if an old script mutated the Media Pool earlier.
         */
        window.mediaPoolManager
            ?.render?.();
    }

    addText() {
        const seq =
            window.ensureSequencerManager(
                this,
                this.project
            );
    ...
    addMedia(entry) {
        ...
        /*
         * Build a tiny runtime draft only to preserve the existing
         * SequencerManager.addMediaClip API. The clip created from it becomes
         * the real project data immediately.
         */
        const draft =
            this.itemAdapter
                ?.createMediaDraft?.(
            ...
            {
                id:
                    `runtime-draft-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
                sourceMediaId:
                    entry.id ||
                    null,
                type: 'media',
        ...
        /*
         * The draft id becomes clip.mediaRef, so the runtime item keeps a
         * stable identity across re-renders without becoming project state.
         */
        this.compositionRuntime
            ?.syncFromProject?.({
                force: true,
        ...
        seq.state.selectClip?.(
            clip,
            false
        );

        seq.setSequencerSelection?.(
            seq.state.selectedIds
        );
    ...
    renderCompositeAt(time) {
        ...
        /*
         * Normally the composition renders to the visible preview canvas with
         * its device-pixel-ratio transform.
         *
         * During Deliver export, renderCompositeToCanvas() installs an
         * explicit transform that maps project coordinates directly to the
         * requested output resolution. This is the key difference between a
         * REAL 4K render and simply upscaling a 720p/1080p preview bitmap.
         */
        const targetTransform =
            this._renderTargetTransform ||
            null;
        ...
        if (targetTransform) {
            ...
            this.ctx.clearRect(
                0,
                0,
                targetTransform.targetWidth,
                targetTransform.targetHeight
            );
        ...
        activeClips.forEach(
            clip => {
                ...
                if (
                    ...
                    activeRuntimeIds.add(
                        item.id
                    );
                }

                /*
                 * Future Fusion / Color / custom compositor stages can hook
                 * here without becoming part of SequencerState.
                 */
                try {
                    window.dispatchEvent(
                        new CustomEvent(
    ...
    /**
     * Render the composition directly into an arbitrary output canvas.
     *
     * Project coordinates stay unchanged. The target context receives a
     * transform that maps those coordinates to the requested output size.
     * Therefore source video/images are sampled directly at output resolution
     * instead of first being rasterized into the editor preview canvas.
     *
     * options.scaleMode:
     *   fit     -> preserve whole composition, letterbox if needed
     *   fill    -> cover output, crop if needed
     *   stretch -> fill output ignoring aspect ratio
     */
    renderCompositeToCanvas(
        ...
        if (
            !targetCanvas ||
            typeof targetCanvas.getContext !==
                'function'
        ) {
            return false;
        }
        ...
        const oldSuppressOverlay =
            this._suppressCanvasOverlay;
    ...
    setMasterGain(value) {
        ...
        if (
            this.project?.audio?.master &&
            this.project.audio.master.gain !==
                this.masterGain
        ) {
            this.project.state.audio.master.gain =
                this.masterGain;
    ...
    getRuntimeItemForClip(clip) {
        if (!clip) return null;

        return this.compositionRuntime
            ?.resolveClip?.(
                clip
            ) ||
            (
