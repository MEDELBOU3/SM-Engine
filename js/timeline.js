...
// === NEW GLOBAL STATE FOR ANIMATION LIBRARY ===
// --- Animation Library Specific State (USING REAL-WORLD PLACEHOLDERS) ---
...
function debouncedResize() {
    clearTimeout(resizeTimeout);
    resizeTimeout = setTimeout(() => {
        if (!isResizing && timelineRes && editorScene) {
            applyPanelHeights(timelineRes.offsetHeight); // Apply current timeline height, it will be constrained
        }
    }, 150); // Wait 150ms after last resize event
}
...
// --- Helper function to apply new heights and update components ---
function applyPanelHeights(newTimelineHeight) {
    if (!timelineRes || !editorScene) {
        console.warn("Missing timelineRes or editorScene for height adjustment.");
        return;
    }

    const dynamicMinHeight = window.innerHeight * MIN_HEIGHT_RATIO;
    const constrainedTimelineHeight = Math.max(dynamicMinHeight, Math.min(MAX_HEIGHT_PX, newTimelineHeight));

    timelineRes.style.height = `${constrainedTimelineHeight}px`;
    editorScene.style.bottom = `${constrainedTimelineHeight}px`; // Adjust editor-scene's bottom
...
// Placeholder for external setters (required for initialization/imports)
function setTimelineDuration(duration) {
    timelineDuration = Math.max(1, duration);
    if (typeof updateTimelineScale === 'function') updateTimelineScale();
}
...
// ====================================================================
// === ANIMATION LIBRARY MANAGER MODULE (Complete Implementation) ===
// ====================================================================
const animationLibraryManager = {
    libraryContainer: document.getElementById('animation-library'),
    animationList: document.getElementById('animation-list'),
    applyButton: document.getElementById('apply-animation'),
    searchField: document.getElementById('animation-search'),

    init() {
        if (!this.libraryContainer) {
            console.warn("Animation Library container not found.");
            return;
        }
        ...
        this.searchField.addEventListener('input', () => {
            this.renderAnimationList(this.searchField.value);
        });
    ...
    renderAnimationList(query = '') {
        if (!this.animationList) return;
        
        this.animationList.innerHTML = '';
        const filtered = availableAnimations.filter(anim => 
            anim.name.toLowerCase().includes(query.toLowerCase())
        );

        if (filtered.length === 0) {
            this.animationList.innerHTML = '<p class="empty-list">No animations found.</p>';
            return;
        }
        ...
        filtered.forEach(anim => {
            ...
            item.innerHTML = `
                <i class="fas fa-running animation-icon"></i>
                <span class="anim-name">${anim.name}</span>
            `;
            item.addEventListener('click', () => this.selectAnimation(anim.id, item));
            this.animationList.appendChild(item);

            if (anim.id === selectedAnimationId) {
                item.classList.add('selected');
            }
        });
    },

    selectAnimation(id, itemElement) {
        document.querySelectorAll('.animation-item').forEach(el => el.classList.remove('selected'));
        itemElement.classList.add('selected');
        selectedAnimationId = id;
        this.applyButton.disabled = !selectedObject;
        console.log(`Selected animation: ${id}, Apply button enabled: ${!this.applyButton.disabled}`);
    },

    /*async applySelectedAnimation() {
        console.log("=== APPLY ANIMATION STARTED ===");
        ...
        const animation = availableAnimations.find(a => a.id === selectedAnimationId);
        if (!animation) {
        ...
        try {
            console.log(`Loading from: ${animation.url}`);
            
            // Load the animation file
            const gltf = await this.loadAnimation(animation.url);
            console.log("GLTF loaded:", gltf);
            console.log("Animations found:", gltf.animations?.length || 0);

            if (!gltf.animations || gltf.animations.length === 0) {
                throw new Error("No animations in this file");
            }

            const clip = gltf.animations[0];
            console.log("Using clip:", clip.name, "Duration:", clip.duration);

            // CRITICAL: Stop and clean everything first
            if (selectedObject.userData.mixer) {
                selectedObject.userData.mixer.stopAllAction();
                selectedObject.userData.mixer.uncacheRoot(selectedObject);
                selectedObject.userData.mixer = null;
            }

            // Clear timeline keyframes
            if (keyframes.has(selectedObject.uuid)) {
                keyframes.delete(selectedObject.uuid);
            }

            // Create NEW mixer and action
            const mixer = new THREE.AnimationMixer(selectedObject);
            const action = mixer.clipAction(clip);
            // CRITICAL: Configure for smooth looping
            action.reset();
            action.setLoop(THREE.LoopRepeat, Infinity);
            action.clampWhenFinished = false;
            action.play();

            action.paused = false; // Let it play naturally

            selectedObject.userData.mixer = mixer;
            selectedObject.userData.animationClip = clip;
            selectedObject.userData.animationAction = action;

            console.log("Mixer created and action configured for looping");
            // Create NEW mixer and action
        

            // Sample for timeline scrubbing
            this.sampleAnimation(selectedObject, clip, mixer);

            // Update timeline duration
            if (clip.duration > timelineDuration) {
                setTimelineDuration(clip.duration);
            }
            ...
            alert(`Animation "${animation.name}" applied! Press Play to see it.`);
            ...
            this.applyButton.textContent = "Apply Animation";
        }
    },*/
    ...
    try {
        ...
        console.log("Loaded animation:", sourceClip.name, "duration:", sourceClip.duration);

        // --- Retarget the animation ---
        const targetSkeleton = selectedObject.getObjectByProperty("type", "SkinnedMesh")?.skeleton;
        const sourceSkeleton = gltf.scene?.getObjectByProperty("type", "SkinnedMesh")?.skeleton;

        let clipToPlay = sourceClip;
        if (sourceSkeleton && targetSkeleton) {
            clipToPlay = this.retargetAnimation(sourceClip, sourceSkeleton, targetSkeleton);
        }
        ...
        const action = selectedObject.userData.mixer.clipAction(clipToPlay);
        action.reset();
        action.setLoop(THREE.LoopRepeat, Infinity);
        action.clampWhenFinished = false;
        action.play();

        selectedObject.userData.animationClip = clipToPlay;
        selectedObject.userData.animationAction = action;

        console.log("Animation applied to selectedObject ✅");
...
retargetAnimation(sourceClip, sourceSkeleton, targetSkeleton) {
    const tracks = [];
    ...
    sourceClip.tracks.forEach(track => {
        ...
        if (targetBone) {
            // Build new track targeting the correct bone
            const newTrackName = track.name.replace(boneName, targetBone.name);

            let TypedArray;
            if (track instanceof THREE.VectorKeyframeTrack) {
                TypedArray = THREE.VectorKeyframeTrack;
            } else if (track instanceof THREE.QuaternionKeyframeTrack) {
                TypedArray = THREE.QuaternionKeyframeTrack;
            } else if (track instanceof THREE.NumberKeyframeTrack) {
                TypedArray = THREE.NumberKeyframeTrack;
            }

            if (TypedArray) {
                const newTrack = new TypedArray(
                    newTrackName,
                    track.times.slice(),
                    track.values.slice()
                );
            ...
            loader.load(
                ...
                (progress) => {
                    if (progress.total > 0) {
                        ...
                        this.applyButton.textContent = `${pct}%`;
                        console.log(`Loading: ${pct}%`);
                    }
                },
                (error) => {
                    console.error("Load error:", error);
                    reject(new Error(`Cannot load: ${error.message}`));
                }
            );
        });
    },

    /*sampleAnimation(character, clip, mixer) {
        console.log("Sampling animation to keyframes...");
        
        const objectKeyframes = {};
        const numSamples = 30;
        
        let target = character.getObjectByName('mixamorigHips');
        if (!target) {
            character.traverse(child => {
                if (!target && child.isBone && !child.parent?.isBone) {
                    target = child;
                }
            });
        }
        ...
        character.userData.animationTarget = target;

        for (let i = 0; i <= numSamples; i++) {
            const time = (i / numSamples) * clip.duration;
            const frame = Math.round(time * (fps || 30));
        ...
        keyframes.set(character.uuid, objectKeyframes);
        console.log(`Sampled ${Object.keys(objectKeyframes).length} keyframes`);
    }*/

        sampleAnimation(character, clip, mixer) {
    console.log("Sampling animation...");
    
    const objectKeyframes = {};
    
    // Sample at actual keyframe times from the animation clip
    const allTimes = new Set();
    clip.tracks.forEach(track => {
        track.times.forEach(t => allTimes.add(t));
    });
    allTimes.add(0);
    ...
    // Save initial pose
    const initPos = target.position.clone();
    const initRot = target.quaternion.clone();
    const initScale = target.scale.clone();

    sortedTimes.forEach(time => {
        const frame = Math.round(time * (fps || 30));
        
        // Reset to bind pose before sampling
        target.position.copy(initPos);
        target.quaternion.copy(initRot);
        target.scale.copy(initScale);
...
function initializeTimeline() {
     animationLibraryManager.init(); // <-- Initialize the new manager
...
function setupTimelineEventListeners() {
    ...
    document.getElementById('add-shatter-modifier').addEventListener('click', () => {
        ...
        modifierManager.addModifier(selectedObject, 'shatter', {
            ...
            explosionForce:25.0,// How fast the pieces fly out
        });
    });
...
function startAnimationDetection() {
    if (isAnimationDetectionEnabled) return;
    
    isAnimationDetectionEnabled = true;
    animationDetectionStartTime = currentTime;
    detectionFrameCounter = 0;
    detectedAnimations.clear();
    
    // Initialize tracking for all scene objects
    scene.traverse((object) => {
        if (object.isObject3D && object.visible && !object.userData.isHelper) {
            const euler = new THREE.Euler().setFromQuaternion(object.quaternion, 'XYZ');
            
            detectedAnimations.set(object.uuid, {
                lastTransform: {
    ...
    document.getElementById('detection-status').innerHTML = '<span class="recording-indicator"></span>Recording...';
    document.getElementById('detection-status').style.color = '#ff4444';
    
    console.log(`Animation detection started. Tracking ${detectedAnimations.size} objects.`);
}

function stopAnimationDetection() {
    if (!isAnimationDetectionEnabled) return;
    
    isAnimationDetectionEnabled = false;
    
    let totalKeyframes = 0;
    let animatedObjects = 0;
    
    // Process all detected animations
    detectedAnimations.forEach((data, uuid) => {
        if (data.frames.length > 0) {
            const object = scene.getObjectByProperty('uuid', uuid);
            if (object) {
                addDetectedAnimationToTimeline(object, data.frames);
                totalKeyframes += data.frames.length;
                animatedObjects++;
            }
        }
    });
    ...
    document.getElementById('detection-status').textContent = `Detection: Off (${totalKeyframes} kfs)`;document.getElementById('detection-status').style.color = '#44ff44';
    
    detectedAnimations.clear();
    
    console.log(`Animation detection stopped. Added ${totalKeyframes} keyframes across ${animatedObjects} objects.`);
}

function addDetectedAnimationToTimeline(object, frames) {
    if (!keyframes.has(object.uuid)) {
        keyframes.set(object.uuid, {});
    }
    
    const objectKeyframes = keyframes.get(object.uuid);
    
    frames.forEach(frame => {
        const frameNumber = Math.round(frame.time * fps);
        
        // Only add if no keyframe exists at this time or if it's significantly different
        const existingKf = objectKeyframes[frameNumber];
        if (!existingKf || hasTransformChanged(existingKf, frame, 0.01)) {
            objectKeyframes[frameNumber] = {
                time: frame.time,
                position: frame.position.clone(),
                rotation: frame.rotation.clone(),
                rotationEuler: frame.rotationEuler,
                scale: frame.scale.clone(),
                interpolation: 'linear'
            };
            
            // Initialize bezier handles
            graphChannels.forEach(channel => {
                objectKeyframes[frameNumber][`handleIn_${channel.name}_x`] = -DEFAULT_BEZIER_HANDLE_X_OFFSET;
                objectKeyframes[frameNumber][`handleIn_${channel.name}_y`] = DEFAULT_BEZIER_HANDLE_Y_OFFSET;
                objectKeyframes[frameNumber][`handleOut_${channel.name}_x`] = DEFAULT_BEZIER_HANDLE_X_OFFSET;
                objectKeyframes[frameNumber][`handleOut_${channel.name}_y`] = DEFAULT_BEZIER_HANDLE_Y_OFFSET;
            });
        }
    });
    
    console.log(`Added ${frames.length} detected keyframes for ${object.name}`);
...
function hasTransformChanged(oldData, newData, threshold = 0.001) {
    if (!oldData.position || !newData.position) return true;
    
    return (
        oldData.position.distanceTo(newData.position) > threshold ||
        Math.abs(oldData.rotation.angleTo(newData.rotation)) > threshold ||
        oldData.scale.distanceTo(newData.scale) > threshold
    );
}


/**
 * Adds animation detection controls to the UI
 */
function addAnimationDetectionControls() {
    const timelineControls = document.querySelector('.playback-controls-group');
    
    // Create detection controls container
    const detectionControls = document.createElement('div');
    detectionControls.className = 'detection-controls-group';
    detectionControls.style.display = 'flex';
    detectionControls.style.alignItems = 'center';
    detectionControls.style.padding = '4px';
    detectionControls.style.backgroundColor = '#4a4a4a';
    detectionControls.style.marginLeft = '8px';
    detectionControls.style.borderRadius = '4px';
    
    detectionControls.innerHTML = `
        <button class="tool-button" id="start-detection" title="Start detecting animations">
            <i class="fas fa-record" style="color: #ff4444;"></i>
        </button>
        <button class="tool-button" id="stop-detection" title="Stop detection and add keyframes" disabled>
            <i class="fas fa-stop"></i>
        </button>
        <span id="detection-status" style="margin-left: 8px; color: #ccc; font-size: 11px; white-space: nowrap;">
            Detection: Off
        </span>
    `;
    
    // Insert after the playback controls group
    const playbackGroup = document.querySelector('.playback-controls-group');
    playbackGroup.parentNode.insertBefore(detectionControls, playbackGroup.nextSibling);
    
    // Add event listeners
    document.getElementById('start-detection').addEventListener('click', startAnimationDetection);
    document.getElementById('stop-detection').addEventListener('click', stopAnimationDetection);
}

// Add CSS for the detection status indicator
const detectionStyle = document.createElement('style');
detectionStyle.textContent = `
...
`;
document.head.appendChild(detectionStyle);

/**
/**
 * Enhanced detection that handles common animation patterns
 */
function enhancedAnimationDetection(object, currentTime) {
    const objectData = detectedAnimations.get(object.uuid);
    ...
    if (hasTransformChanged(objectData.lastTransform, currentTransform)) {
        const euler = new THREE.Euler().setFromQuaternion(object.quaternion, 'XYZ');
        
        // Only record if enough time has passed since last keyframe (avoid duplicates)
        const lastFrame = objectData.frames[objectData.frames.length - 1];
        const minFrameInterval = 1 / fps; // Minimum time between keyframes
        
        if (!lastFrame || (currentTime - lastFrame.time) >= minFrameInterval) {
            objectData.frames.push({
                time: currentTime,
            ...
            objectData.lastTransform = currentTransform;
            objectData.hasChanged = true;
        }
    }
}

function updateAnimationDetection(delta) {
    if (!isAnimationDetectionEnabled || !isPlaying) return;
    
    detectionFrameCounter++;
    
    // Sample at reduced rate to avoid too many keyframes
    if (detectionFrameCounter % DETECTION_SAMPLE_RATE !== 0) return;
    
    scene.traverse((object) => {
        if (!object.isObject3D || !object.visible || object.userData.isHelper) return;
        
        const objectData = detectedAnimations.get(object.uuid);
        if (!objectData) {
            // New object appeared during detection
            const euler = new THREE.Euler().setFromQuaternion(object.quaternion, 'XYZ');
            detectedAnimations.set(object.uuid, {
                lastTransform: {
                    position: object.position.clone(),
        ...
        if (hasTransformChanged(objectData.lastTransform, currentTransform, 0.001)) {
            const euler = new THREE.Euler().setFromQuaternion(object.quaternion, 'XYZ');
            
            // Only record if enough time has passed since last keyframe
            const lastFrame = objectData.frames[objectData.frames.length - 1];
            const minFrameInterval = 0.1; // Minimum 0.1 seconds between keyframes
            ...
            if (!lastFrame || (currentTime - lastFrame.time) >= minFrameInterval) {
                ...
                // Update detection status with live count
                const totalFrames = Array.from(detectedAnimations.values())
                    .reduce((sum, data) => sum + data.frames.length, 0);
                document.getElementById('detection-status').innerHTML = 
                    `<span class="recording-indicator"></span>Recording... (${totalFrames} kfs)`;
            }
        }
    });
}
...
document.addEventListener('keydown', (e) => {
    ...
    // Animation detection shortcuts
    if (e.ctrlKey && e.key === 'r' && !e.repeat) { // Ctrl+R to start/stop detection
        e.preventDefault();
        if (isAnimationDetectionEnabled) {
