...
function setupTimelineEventListeners() {
    ...
    timelineContent.addEventListener('click', (e) => {
        if (e.target.classList.contains('keyframe')) {
            ...
            // This needs to account for zoom level and scroll offset
...
function addObjectToTimeline(object) {
    ...
    // Pass 1: Gather all raw tracks and find every unique timestamp.
    object.animations.forEach(clip => {
        clip.tracks.forEach(track => {
            // Find the top-level transform tracks. You can extend this for individual bones later.
            ...
            else return; // Skip bone tracks for now
    ...
    // Pass 2: Iterate through a sorted list of unique times and create a *complete* keyframe at each point.
...
function getValueAtTime(time, track, defaultValue, outValue) {
    ...
    if (time < times[0]) return outValue.fromArray(values, 0);
    if (time >= times[times.length - 1]) return outValue.fromArray(values, (times.length - 1) * valueSize);
    for (let i = 0; i < times.length - 1; i++) {
        if (time >= times[i] && time < times[i + 1]) {
            ...
            return outValue.isQuaternion ? THREE.Quaternion.slerp(v0, v1, outValue, t) : outValue.copy(v0).lerp(v1, t);
        }
    }
    return outValue.copy(defaultValue); // Should not be reached
}
...
function setupModelInScene(object, name) {
    if (!object) {
        console.error("setupModelInScene was called with a null object.");
        return;
    }
    ...
    // 1. Set standard properties like shadows on the model's meshes
    object.traverse((child) => {
        if (child.isMesh) {
            child.castShadow = true;
            child.receiveShadow = true;
    ...
    // 3. ✅ FIX: Immediately set it as the selected object.
    selectedObject = object; 
    
    // 4. Attach transform controls to the new selection.
    if (transformControls) {
        transformControls.attach(object);
    }

    // 5. Correctly handle the timeline for EITHER animated OR static models.
    if (object.animations && object.animations.length > 0) {
        // This is an ANIMATED model.
        console.log(`Found ${object.animations.length} animations for '${object.name}'. Setting up mixer and timeline.`);
        
        // Setup the AnimationMixer for playback in the 3D view.
        object.userData.mixer = new THREE.AnimationMixer(object);
        object.animations.forEach(clip => {
            object.userData.mixer.clipAction(clip).play();
        });
        
        // Use the new, robust functions to populate the timeline.
        autoSetTimelineDurationFromAnimations(object);
        addObjectToTimeline(object); 
    } else {
        // This is a STATIC model.
        console.log(`'${object.name}' is a static model. Adding initial keyframe to timeline.`);
        // Add a single keyframe so it appears on the timeline for manual animation.
        addKeyframe();
    }

    // 6. ✅ FIX: Finally, update all relevant UI panels.
    updateLayersUI();
    if (typeof updateHierarchy === 'function') {
        updateHierarchy(); // Make sure this function exists and works
    }
}
...
function applyKeyframe(object, keyframe) {
    ...
    object.quaternion.copy(keyframe.rotation); // ✅ CORRECT: Use quaternion for rotation
