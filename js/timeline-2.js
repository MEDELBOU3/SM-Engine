...
function handleGraphMouseDown(e) {
    ...
    for (let i = channelData.length - 1; i >= 0; i--) {
        ...
        if (kf.interpolation === 'bezier') {
            ...
            if (distOut < 8) { // Increased hitbox for easier selection
            ...
            if (distIn < 8) { // Increased hitbox for easier selection
                selectedGraphKeyframe = i;
        ...
        if (dist < 10) { // Increased hitbox for easier selection
            selectedGraphKeyframe = i;
...
/*function updateGraphKeyframePosition(mouseX, mouseY) {
    ...
    if (newFrame !== kfToUpdate.frame) {
        ...
        // Update the specific channel's value (position.x, scale.y, etc.) on this new data object
        const channelObj = graphChannels.find(c => c.name === selectedGraphChannel);
        if (channelObj) {
            const [prop, subProp] = channelObj.name.split('.');
            if (movedKfData[prop] && movedKfData[prop][subProp] !== undefined) {
                 movedKfData[prop][subProp] = graphPos.value;
            } else if (prop === 'rotation') {
                console.warn(`Direct graph editing of rotation components on Quaternions is not fully supported in this editor's current implementation.`);
                // For proper rotation editing via graph, kfData should store Euler angles.
                // If you want to implement this, you'd convert `movedKfData.rotation` to Euler,
                // update the `subProp` (e.g., `.x`), and then convert back to Quaternion.
            }
        }
        ...
        if (channelObj) {
            const [prop, subProp] = channelObj.name.split('.');
            if (originalKfData[prop] && originalKfData[prop][subProp] !== undefined) {
    ...
    // No need to re-set the `keyframes` map itself, as `originalObjectKeyframes` is a direct reference.
}*/ 
...
function moveMarker(id, newTimeSec) {
    const m = timelineMarkers.find(x => x.id === id); // ✅ corrected
    if (m) {
        m.time = Math.max(0, Math.min(timelineDuration, newTimeSec));
        renderMarkers();
    }
