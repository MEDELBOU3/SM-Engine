...
// ---------- STAIRS ADVANCED: GLOBAL STATE ----------
/*let stairDraft = null;          // runtime preview+params while placing
let stairPreviewGroup = null;   // Group for live preview steps
let stairActiveGroup = null;    // Last created stairs (for edit)
let isStairRotating = false;
...
let stairDefaults = {
  ...
  stepDepth: 0.28,
  // Advanced
  turnType: 'straight',         // 'straight' | 'L' | 'U'
  turnPosition: 0.5,            // 0..1 along the run
  landingDepth: 1.0,            // depth of landing measured along run
  yawOffsetDeg: 0,              // additional yaw (mouse wheel / R)
  snapAngleDeg: 15,             // for Ctrl snapping while rotating
  minStep: 0.05,                // guard
  railing: false,               // (placeholder switch)
  stringers: false              // (placeholder switch)
};*/
...
function buildQuadMap(geometry) {
    ...
    for (let faceIndex1 = 0; faceIndex1 < indices.length / 3; faceIndex1++) {
        ...
        for (const edgeKey of tri1_edges) {
            ...
            if (processedFaces.has(faceIndex2)) continue;

            // We found a potential pair. Check if they are co-planar.
            ...
            if (normal1.dot(normal2) > 0.999) {
                ...
                if (quad_indices.length === 4) {
                    ...
                    // Store the quad vertices in a predictable order (e.g., outer1 -> shared1 -> outer2 -> shared2)
...
function handleCanvasClick(event) {
    ...
    if (activeArchTool) {
        switch (activeArchTool) {
            ...
            case 'boolean-subtract': 
                // Boolean logic is special, it needs to select things, so we handle it separately
                handleBooleanSelectionClick(event); // A dedicated function is cleaner
                break;
    ...
    /*if (!isModelingMode || (isModelingMode && !activeArchTool && !isLoopCutMode && !splineCreationMode)) {
        ...
        if (!isModelingMode) { // Simplified check: only select arch elements if not in modeling mode
        const intersects = raycaster.intersectObjects(architecturalElements, true); // Intersect children too
        if (intersects.length > 0) {
            let clickedElement = intersects[0].object;
            // Traverse up to find the root architectural element
    ...
    if (isModelingMode) {
        // This block only runs if isModelingMode is true AND no arch tool was active
        if (splineCreationMode) {
            // handleSplineDrawClick(event);
        } else {
            onModelingClick(event); // Your function for selecting vertices, edges, faces
        }
    }
}*/
 ...
 */
function handleCanvasRightClick(event) {
    // Since this listener only effectively runs in modeling mode (due to the check in the other script),
    // we know we need to take control and prevent the default browser menu.
    event.preventDefault();

    // --- PRIORITY 1: Cancel any active multi-stage tool ---
    // If a tool is active, its cancellation takes priority over showing the menu.
    ...
    if (polyPenToolActive) {
        ...
        if (polyPenPreviewFace) polyPenPreviewFace.visible = false;
        console.log("Poly Pen: Canceled current face chain.");
        return; // Event handled, do nothing more.
    }
    if (structureSynthToolActive) {
        handleSynthRightClick(event);
    ...
    if (activeArchTool) {
        if (activeArchTool === 'stairs' && stairCreationStage > 0) {
            cleanupStairsTool();
        } else if (activeArchTool === 'wall' && wallPoints.length > 0) {
            finishWall();
        } else if (activeArchTool === 'roof' && roofPoints.length > 0) {
            createRoof();
        } else if (activeArchTool === 'curved-wall' && curvedWallPoints.length > 0) {
            (curvedWallPoints.length < 3) ? deactivateCurrentArchTool() : createCurvedWall();
        } else {
            deactivateCurrentArchTool();
        }
        return; // Event handled.
    ...
    // This code only runs if none of the 'return' statements above were hit.
    if (isModelingMode && !isTransforming) {
        // Hide the other menu just to be absolutely safe
        const globalMenu = document.getElementById('context-menu');
        if (globalMenu) globalMenu.style.display = 'none';
        
        // Now show the modeling menu
...
// REPLACE your existing clearMeshStructure function with this one.
...
function clearMeshStructure() {
    [vertexHelpers, edgeHelpers, faceHelpers].forEach(group => {
        // Iterate backwards when removing children to avoid index shifting issues
        for (let i = group.children.length - 1; i >= 0; i--) {
            const child = group.children[i];
            group.remove(child); // Remove from group
 ...
 * This is a custom, self-contained implementation.
 * @param {THREE.BufferGeometry} geometry The geometry to process.
 * @param {number} [tolerance=1e-5] (Not used in this key-based method, precision is from toFixed)
 * @returns {THREE.BufferGeometry} A new, optimized BufferGeometry.
 */
function mergeVerticesByPosition(geometry, tolerance = 1e-5) {
    const positions = geometry.attributes.position.array;
    const indexAttr = geometry.index ? geometry.index.array : null;
    ...
    const newVertices = [];
    const vertexMap = new Map(); // key: rounded position string -> new vertex index
    const indexRemap = new Array(vertCount); // old vertex index -> new vertex index

    // Create a map from unique positions to new vertex indices
    for (let i = 0; i < vertCount; i++) {
        const x = positions[i * 3];
        const y = positions[i * 3 + 1];
        const z = positions[i * 3 + 2];
    ...
    // Build the new index array using the remapped indices
    let newIndexArray = [];
    if (indexAttr) {
    ...
    // Filter out degenerate triangles (where two or more vertices are the same)
    const filteredIndex = [];
    for (let i = 0; i < newIndexArray.length; i += 3) {
        const a = newIndexArray[i];
        const b = newIndexArray[i + 1];
    ...
    // Create and return the final, clean geometry
    const newGeo = new THREE.BufferGeometry();
...
function updateEdgeFaceHelpers() {
    ...
    // --- EDGE HELPERS (No change needed for edges) ---
        ...
        if (indices) {
            for (let i = 0; i < indices.length; i += 3) {
                for (let j = 0; j < 3; j++) {
                    ...
                    if (!edgeSet.has(edgeKey)) {
                        edgeSet.add(edgeKey);
                        const vStart = new THREE.Vector3().fromBufferAttribute(positions, startIdx).applyMatrix4(matrix);
                        const vEnd = new THREE.Vector3().fromBufferAttribute(positions, endIdx).applyMatrix4(matrix);
                        const edgeGeom = new THREE.BufferGeometry().setFromPoints([vStart, vEnd]);
                        const edgeMat = new THREE.LineBasicMaterial({ color: 0xE67E22, linewidth: edgeThickness });
                        const edge = new THREE.Line(edgeGeom, edgeMat);
                        edge.userData = { type: 'edge', indices: [startIdx, endIdx] };
                        edgeHelpers.add(edge);
    ...
    // --- FACE HELPERS (NEW QUAD-AWARE LOGIC) ---
    if (selectionMode === 'face' || selectionMode === 'all') {
        const quadMap = buildQuadMap(geometry);
        const tempVerts = [];

        quadMap.forEach((faceVertexIndices, originalFaceIndex) => {
            tempVerts.length = 0; // Clear the array
            ...
            faceMesh.userData = {
                ...
                faceIndex: originalFaceIndex // Store the original triangle index as an ID
            };
            faceHelpers.add(faceMesh);
        });
    }
...
/*function initiateSpinTool() {
    // Check if a profile exists. If not, give the user helpful instructions.
    if (!editedSplines.profile) {
        alert("Spin Tool Error: Please select a profile first.\n\nHow to create a profile:\n1. Select a continuous chain of edges in Edge Mode.\n2. Click the 'Edges to Profile' button.");
        return;
    }
    
    // If a profile exists, activate the tool as before.
    toggleArchTool('spin');
}*/  
...
// This should be called by toggleArchTool when 'spin' is selected
/*function initSpinTool() {
    console.log("Spin Tool Initialized");
    if (!spinPreviewMesh) {
        const material = new THREE.MeshStandardMaterial({
            color: 0x00ffff,
            roughness: 0.4,
    ...
    if (!spinAxisLine) {
        ...
        const points = [new THREE.Vector3(0, -1000, 0), new THREE.Vector3(0, 1000, 0)];
        const geometry = new THREE.BufferGeometry().setFromPoints(points);
        spinAxisLine = new THREE.Line(geometry, material);
        spinAxisLine.renderOrder = 998;
        spinAxisLine.name = "SpinAxis";
        scene.add(spinAxisLine);
    }
    
    // Position axis at the start of the profile spline for a default pivot
    if (editedSplines.profile && editedSplines.profile.points.length > 0) {
        const firstPoint = editedSplines.profile.points[0];
        spinPivotPoint.set(firstPoint.x, firstPoint.y, 0); // Assume profile is on XY plane
    } else {
        spinPivotPoint.set(0,0,0);
    }
    spinAxisLine.position.copy(spinPivotPoint);
    spinAxisLine.visible = true;

    // Add listener to the 'Create' button in the spin options panel
    document.getElementById('executeSpinButton').onclick = createSpinGeometry;
    
    // Add listeners to UI inputs to update the preview in real-time
    document.getElementById('spinSegmentsInput').oninput = updateSpinPreview;
    document.getElementById('spinAngleInput').oninput = updateSpinPreview;

    updateSpinPreview(); // Generate initial preview
}

function updateSpinPreview() {
    if (!spinPreviewMesh || !editedSplines.profile) return;
    ...
    // The LatheGeometry revolves points around the Y-axis. Our profile is in XY.
    // The points in the profile are relative to the spin axis.
    const profilePoints = editedSplines.profile.points.map(p => {
        // Translate profile points so they are relative to the pivot
        return new THREE.Vector2(p.x - spinPivotPoint.x, p.y - spinPivotPoint.y);
    });
    ...
    const newGeometry = new THREE.LatheGeometry(profilePoints, segments, 0, phiLength);
    
    spinPreviewMesh.geometry.dispose();
    spinPreviewMesh.geometry = newGeometry;
    spinPreviewMesh.position.copy(spinPivotPoint); // Position the final mesh at the pivot point
...
function createSpinGeometry() {
    ...
    // Create the final mesh from the preview's geometry
    ...
    const oldGeometry = new THREE.BufferGeometry(); // Create empty geometry for history
    
    addObjectToScene(finalMesh, "spin-result");
    
    if (window.historyManager) {
        // For a created object, it's an "add" action, but geometryChange can also work
        // A dedicated "add object" history would be better. For now, we'll log it.
        console.log("Spin geometry created. Add 'addObject' to your history manager for full undo.");
    }

    console.log("Spin geometry created:", finalMesh.name);
    deactivateCurrentArchTool(); // Exit the spin tool
}*/ 
...
function initSpinTool() {
    ...
    if (!spinAxisLine) {
        const material = new THREE.LineBasicMaterial({ color: 0xff00ff, depthTest: false });
        const points = [new THREE.Vector3(0, -1000, 0), new THREE.Vector3(0, 1000, 0)];
        spinAxisLine = new THREE.Line(new THREE.BufferGeometry().setFromPoints(points), material);
        scene.add(spinAxisLine);
    }

    // *** THE FIX ***
    // Use the 3D pivot stored from the 'Edges to Profile' conversion
    if (editedSplines.profile.userData.pivot3D) {
        spinPivotPoint.copy(editedSplines.profile.userData.pivot3D);
    } else {
        // Fallback to the first point if pivot is missing (e.g., from old spline drawing)
        const firstPoint = editedSplines.profile.points[0];
        spinPivotPoint.set(firstPoint.x, firstPoint.y, 0);
    }

    // Position the visual axis line at the object's 3D center
    spinAxisLine.position.copy(spinPivotPoint);
    // Align axis with the world Y axis by default. Can be made interactive later.
    spinAxisLine.quaternion.identity(); 
    spinAxisLine.visible = true;
