...
class Storyboard2DManager {
    ...
    getTemplateHTML() {
        return `
            <!-- Professional Storyboard Toolbar -->
            <div class="storyboard-toolbar">
                <div class="storyboard-toolbar-group">
                    <button class="storyboard-btn storyboard-btn-icon" id="sb-new-frame" title="New Frame"><i class="fas fa-plus"></i></button>
                    <button class="storyboard-btn storyboard-btn-icon" id="sb-delete-frame" title="Delete Frame"><i class="fas fa-trash"></i></button>
                    <button class="storyboard-btn storyboard-btn-icon" id="sb-duplicate-frame" title="Duplicate"><i class="fas fa-clone"></i></button>
                    
                    <div class="storyboard-toolbar-divider"></div>
                    
                    <button class="storyboard-btn storyboard-btn-icon" id="sb-undo" title="Undo"><i class="fas fa-undo"></i></button>
                    <button class="storyboard-btn storyboard-btn-icon" id="sb-redo" title="Redo"><i class="fas fa-redo"></i></button>
                    
                    <div class="storyboard-toolbar-divider"></div>
                    
                    <button class="storyboard-btn storyboard-btn-icon active" id="sb-view-grid" title="Grid"><i class="fas fa-th"></i></button>
                    <button class="storyboard-btn storyboard-btn-icon" id="sb-view-list" title="List"><i class="fas fa-list"></i></button>
                    <button class="storyboard-btn storyboard-btn-icon" id="sb-view-detail" title="Detail"><i class="fas fa-window-maximize"></i></button>
                    
                    <div class="storyboard-toolbar-divider"></div>
                    
                    <button class="storyboard-btn storyboard-btn-icon" id="sb-import" title="Import"><i class="fas fa-download"></i></button>
                    <button class="storyboard-btn storyboard-btn-icon" id="sb-export" title="Export"><i class="fas fa-upload"></i></button>
                    
                    <div class="storyboard-toolbar-divider"></div>
                    
                    <div class="storyboard-input-group">
                        <label>FPS</label>
                        <input type="number" id="sb-fps-input" min="12" max="60" value="24" style="width: 45px;">
                    </div>
                    
                    <div class="storyboard-input-group">
                        <label>Dur</label>
                        <input type="number" id="sb-duration-input" min="0.1" max="30" step="0.1" value="2.0" style="width: 50px;">
                    </div>
                    
                    <div class="storyboard-toolbar-divider"></div>
                    
                    <button class="storyboard-btn" id="sb-settings"><i class="fas fa-cog"></i></button>
                    <button class="storyboard-btn" id="sb-help"><i class="fas fa-question-circle"></i></button>
            ...
            <div class="storyboard-content">
                <!-- Grid View (Default) -->
                <div class="storyboard-grid-container" id="storyboard-grid-container" style="display: flex;">
                ...
                <div class="storyboard-detail-view" id="storyboard-detail-view" style="display: none;">
                    ...
                    <div class="storyboard-detail-section">
                        ...
                        <div class="storyboard-detail-field">
                            <label class="storyboard-detail-label">Duration</label>
                            <input type="number" class="storyboard-detail-input" id="detail-duration" step="0.1">
                        </div>
                        <div class="storyboard-detail-field">
                            <label class="storyboard-detail-label">Notes</label>
                            <textarea class="storyboard-detail-input" id="detail-notes" style="height: 80px; resize: vertical;"></textarea>
                        </div>
                        <div class="storyboard-detail-field">
                            <label class="storyboard-detail-label">Camera</label>
                            <textarea class="storyboard-detail-input" id="detail-camera" style="height: 60px; resize: vertical;"></textarea>
                        </div>
                        <div class="storyboard-detail-field">
            ...
            <div class="storyboard-timeline" id="storyboard-timeline">
                <div class="storyboard-timeline-playback">
                    <button class="storyboard-timeline-btn" id="sb-play" title="Play"><i class="fas fa-play"></i></button>
                    <button class="storyboard-timeline-btn" id="sb-pause" title="Pause"><i class="fas fa-pause"></i></button>
                    <button class="storyboard-timeline-btn" id="sb-stop" title="Stop"><i class="fas fa-stop"></i></button>
                </div>
                ...
                <span id="sb-timeline-info" style="font-size: 9px; color: var(--sb-text-muted); width: 70px;">0:00 / 0:00</span>
            </div>
        `;
    }
    ...
    createFrameCard(frameData) {
        ...
        card.innerHTML = `
            <div class="storyboard-frame-header">
                <span class="storyboard-frame-number">#${frameData.number}</span>
                <span class="storyboard-frame-time">${frameData.duration.toFixed(1)}s</span>
            </div>

            <div class="storyboard-frame-canvas-wrapper">
                ${frameData.previewImage 
                    ? `<img class="storyboard-frame-image" src="${frameData.previewImage}" alt="Frame ${frameData.number}">` 
                    : '<canvas class="storyboard-frame-canvas"></canvas><div class="storyboard-canvas-placeholder"><i class="fas fa-image"></i></div>'
                }
            </div>

            <div class="storyboard-frame-meta">
                <div class="storyboard-frame-meta-item">
                    <span class="storyboard-meta-label">Scene:</span>
                    <span class="storyboard-meta-value">${frameData.sceneName.substring(0, 20)}</span>
                </div>
            </div>

            <div class="storyboard-frame-actions">
                <button class="storyboard-frame-action-btn" data-action="edit" title="Edit"><i class="fas fa-pencil-alt"></i></button>
                <button class="storyboard-frame-action-btn" data-action="duplicate" title="Duplicate"><i class="fas fa-copy"></i></button>
                <button class="storyboard-frame-action-btn" data-action="delete" title="Delete"><i class="fas fa-trash"></i></button>
            </div>
        `;
        ...
        this.frames.forEach((frameData, frameId) => {
            ...
            listItem.innerHTML = `
                <div class="storyboard-list-thumbnail">
                    ${frameData.previewImage 
                        ? `<img src="${frameData.previewImage}" alt="Frame ${frameData.number}">`
                        : '<canvas></canvas><div class="storyboard-canvas-placeholder"><i class="fas fa-image"></i></div>'
                    }
                </div>

                <div class="storyboard-list-info">
                    <div class="storyboard-list-title">${frameData.sceneName}</div>
                    <div class="storyboard-list-meta">
                        <span>${frameData.duration.toFixed(1)}s</span>
                        <span>•</span>
                        <span>${frameData.tags.length} tag${frameData.tags.length !== 1 ? 's' : ''}</span>
                    </div>
                </div>
