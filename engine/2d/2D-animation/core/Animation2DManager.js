...
class Animation2DManager {
    constructor() {
        ...
        this.brushPresets = [
            { id: 'pencil_hb', name: 'HB', icon: 'HB', thumb: 'assets/brushs-images/HB-br.png', category: 'draw', radius: 8, strength: 0.78, spacing: 0.14, jitter: 0.02, hardness: 0.9, streamline: 0.45, taperStart: 0.02, taperEnd: 0.16 },
            { id: 'ink_pen', name: 'Ink', icon: 'INK', thumb: 'assets/brushs-images/Ink-br.png', category: 'draw', radius: 5, strength: 0.95, spacing: 0.08, jitter: 0.01, hardness: 0.98, streamline: 0.75, taperStart: 0.08, taperEnd: 0.3 },
            { id: 'marker_soft', name: 'Marker', icon: 'M', thumb: 'assets/brushs-images/Marker-br.png', category: 'draw', radius: 16, strength: 0.65, spacing: 0.22, jitter: 0.05, hardness: 0.55, streamline: 0.35, taperStart: 0, taperEnd: 0 },
            { id: 'calligraphy', name: 'Callig', icon: 'S', thumb: 'assets/brushs-images/Callig-br.png', category: 'draw', radius: 13, strength: 0.85, spacing: 0.12, jitter: 0.01, hardness: 0.92, streamline: 0.6, taperStart: 0.06, taperEnd: 0.26 },
            { id: 'hatch', name: 'Hatch', icon: '///', thumb: 'assets/brushs-images/hatch-br.png', category: 'draw', radius: 4, strength: 0.72, spacing: 0.32, jitter: 0.03, hardness: 0.95, streamline: 0.4, taperStart: 0, taperEnd: 0.1 },
            { id: 'spray', name: 'Spray', icon: 'SP', thumb: 'assets/brushs-images/spray-br.png', category: 'draw', radius: 20, strength: 0.5, spacing: 0.08, jitter: 0.1, hardness: 0.4, streamline: 0.25, taperStart: 0, taperEnd: 0, forceTool: 'spray' },
            { id: 'smudge_soft', name: 'Soft', icon: '~', thumb: 'assets/brushs-images/soft.png', category: 'utility', radius: 20, strength: 0.35, spacing: 0.24, jitter: 0.08, hardness: 0.25, streamline: 0.2, taperStart: 0, taperEnd: 0 },
            { id: 'eraser_hard', name: 'Erase', icon: 'E', thumb: 'assets/brushs-images/Erase-br.png', category: 'erase', radius: 18, strength: 1, spacing: 0.18, jitter: 0, hardness: 0.98, streamline: 0.55, taperStart: 0, taperEnd: 0, forceTool: 'eraser' },
            { id: 'eraser_soft', name: 'E-Soft', icon: 'ES', thumb: 'assets/brushs-images/Esouft-br.png', category: 'erase', radius: 26, strength: 0.7, spacing: 0.2, jitter: 0.02, hardness: 0.45, streamline: 0.35, taperStart: 0, taperEnd: 0, forceTool: 'eraser' },
            { id: 'fill_util', name: 'Fill', icon: 'F', thumb: 'assets/brushs-images/Fill-br.png', category: 'utility', radius: 12, strength: 1, spacing: 0.1, jitter: 0, hardness: 1, streamline: 0.5, taperStart: 0, taperEnd: 0, forceTool: 'fill' }
        ];
    ...
    _captureViewportEnvironment() {
        if (this.savedViewportState.environmentCaptured) return;

        const scene = window.scene;
        const renderer = window.renderer;
        const THREE = window.THREE;
        const clearColor = renderer?.getClearColor && THREE?.Color
            ? renderer.getClearColor(new THREE.Color()).clone()
            : null;

        this.savedViewportState.sceneBackground = scene?.background;
        this.savedViewportState.sceneFog = scene?.fog;
        this.savedViewportState.rendererClearColor = clearColor;
        this.savedViewportState.rendererClearAlpha = renderer?.getClearAlpha?.();
        this.savedViewportState.environmentCaptured = true;
    }

    _applyWhiteWorkspaceBackground() {
        const scene = window.scene;
        const renderer = window.renderer;
        const THREE = window.THREE;
        if (!scene || !THREE?.Color) return;

        window.__sm2DWhiteViewport = true;
        ...
        [sky?._sky, sky?._cloudMesh, sky?._starsMesh].forEach(object => {
            if (object) object.visible = false;
        });
    }

    _restoreViewportEnvironment() {
        const state = this.savedViewportState;
        if (!state.environmentCaptured) return;

        window.__sm2DWhiteViewport = false;
        if (window.scene) {
            window.scene.background = state.sceneBackground;
            window.scene.fog = state.sceneFog;
            delete window.scene._saved2dBackground;
        }
        if (window.renderer?.setClearColor && state.rendererClearColor) {
            window.renderer.setClearColor(
                state.rendererClearColor,
                state.rendererClearAlpha ?? 1
            );
        }
        state.environmentCaptured = false;
    ...
    setupListeners() {
        ...
        // Pointer events provide pressure and coalesced samples while keeping one render per frame.
        ...
        this.canvas.addEventListener('pointerdown', (event) => {
            ...
            this._historyBeforeState = window.historyManager ? window.historyManager.capture2DState() : null;
            try {
                this.onMouseDown(event);
            } catch (error) {
                console.error('[Animation2D] pointerdown failed:', error);
                this.isDrawing = false;
                this.currentStroke = null;
            }
        }, { passive: false });
        this.canvas.addEventListener('pointermove', (event) => {
            ...
            const samples = raw.length ? raw : [event];
            const maxSamples = this._maxCoalescedSamples || 4;

            if (samples.length <= maxSamples) {
                for (let i = 0; i < samples.length; i++) {
                    this.onMouseMove(samples[i]);
                }
                return;
            }
            ...
            if (last !== samples[samples.length - 1 - ((samples.length - 1) % step)]) {
                this.onMouseMove(last);
            }
        });
        const finishPointer = (event) => {
            ...
            if (this._historyBeforeState && window.historyManager && !window.historyManager.isExecuting) {
                const afterState = window.historyManager.capture2DState();
                if (afterState && JSON.stringify(this._historyBeforeState.keyframes) !== JSON.stringify(afterState.keyframes)) {
                    window.historyManager.addStep({
                        type: '2d_stroke',
                        name: `2D Stroke (Frame ${this.getCurrentFrameIndex()})`,
                        before: this._historyBeforeState,
                        after: afterState
                    });
                }
            }
            this._historyBeforeState = null;
        };
        ...
        this.canvas.addEventListener('lostpointercapture', (event) => {
            if (this.isDrawing) finishPointer(event);
        });
    ...
    resize() {
        ...
        // Layout can publish a resize before the 2D panel has been mounted.
        // Never ask Canvas2D/post-FX to draw a zero-sized source canvas.
        // Post-processing creates quarter-resolution buffers. A transient
        // 1-3px layout size would truncate those buffers to 0x0 and make
        // drawImage throw, aborting the entire 3D editor initialization.
        ...
        try {
            ...
            // 2D is an optional workspace and must never abort the 3D engine
            // bootstrap because a browser resized an off-screen canvas.
            if (!this._resizeRenderWarningShown) {
                this._resizeRenderWarningShown = true;
                console.warn(
                    '[Animation2D] Deferred render until the canvas is ready:',
                    error
                );
            }
            return false;
    ...
    _cloneStrokeData(data) {
        return JSON.parse(JSON.stringify(Array.isArray(data) ? data : []));
    }

    _pressureForEvent(event, distance = 0) {
        const pressure = Number(event?.pressure);
        if ((event?.pointerType === 'pen' || event?.pointerType === 'touch') && pressure > 0) {
            return Math.max(0.05, Math.min(1, pressure));
        }
        const velocityPressure = 1 / (1 + Math.max(0, distance) * 0.055);
        return Math.max(0.35, Math.min(0.95, 0.48 + velocityPressure * 0.42));
    }

    _scheduleStrokeRender() {
        if (this._strokeRenderRequest) return;

        this._strokeRenderRequest = requestAnimationFrame(() => {
            this._strokeRenderRequest = 0;
            ...
            if (
                this._strokeBaseCanvas &&
                this._strokeBaseCanvas.width === this.canvas.width &&
                this._strokeBaseCanvas.height === this.canvas.height
            ) {
                ctx.drawImage(this._strokeBaseCanvas, 0, 0);
            } else {
                // Fallback: rebuild the frame without depending on the optional
                // stroke-base buffer.
                ctx.restore();
                this.render();
                // render() rebuilds the committed frame; draw the live stroke on top.
    ...
    _cancelStrokeRender() {
        if (!this._strokeRenderRequest) return;
        cancelAnimationFrame(this._strokeRenderRequest);
        this._strokeRenderRequest = 0;
    }

    // The advanced manager customizes this for post-processing, but the base
    // drawing workspace must never depend on that optional module.  Without a
    // base implementation pointer-down threw before a stroke could begin.
    _captureStrokeBase() {
        const width = this.canvas?.width || 0;
        const height = this.canvas?.height || 0;
        if (width < 1 || height < 1) return;

        if (!this._strokeBaseCanvas) {
        ...
        if (!this._strokeBaseCtx) return;

        if (this._strokeBaseCanvas.width !== width) this._strokeBaseCanvas.width = width;
        if (this._strokeBaseCanvas.height !== height) this._strokeBaseCanvas.height = height;

        this._strokeBaseCtx.setTransform(1, 0, 0, 1, 0, 0);
        this._strokeBaseCtx.globalAlpha = 1;
        this._strokeBaseCtx.globalCompositeOperation = 'source-over';
        this._strokeBaseCtx.clearRect(0, 0, width, height);
        this._strokeBaseCtx.drawImage(this.canvas, 0, 0);
    }
    ...
    onMouseMove(e) {
        ...
        if (this.currentTool === 'select' && this.selectionDragLast) {
            this.dragSelectedStroke(e);
            return;
        }
        this.draw(e);
    }
    ...
    startStrokeSelection(e) {
        if (e.button !== 0) return;
        const pos = this.getMousePos(e);
        const hitRadius = 10 / Math.max(0.2, this.view2D.scale || 1);
        this.selectedStroke = null;

        for (let i = this.strokes.length - 1; i >= 0; i--) {
            const stroke = this.strokes[i];
            const layer = this.layers.find((item) => item.id === (stroke.layerId || 'layer_0'));
            if (layer && (!layer.visible || layer.locked)) continue;
            const threshold = hitRadius + Math.max(2, (stroke.size || 1) * 0.5);
            const points = stroke.points || [];
            if (points.some((point) => Math.hypot(point.x - pos.x, point.y - pos.y) <= threshold)) {
                this.selectedStroke = stroke;
                break;
            }
            if (stroke.fillRect) {
                const rect = stroke.fillRect;
        ...
        this.selectionDragLast = this.selectedStroke ? pos : null;
        this.render();
    }

    dragSelectedStroke(e) {
        if (!this.selectedStroke || !this.selectionDragLast) return;
        const pos = this.getMousePos(e);
        const dx = pos.x - this.selectionDragLast.x;
        const dy = pos.y - this.selectionDragLast.y;
        (this.selectedStroke.points || []).forEach((point) => {
            point.x += dx;
            point.y += dy;
        });
        if (this.selectedStroke.fillRect) {
            this.selectedStroke.fillRect.x += dx;
            this.selectedStroke.fillRect.y += dy;
        }
        this.selectionDragLast = pos;
        this.render();
    }

    stopStrokeSelection() {
        if (this.selectedStroke && this.selectionDragLast) this.saveKeyframe();
        this.selectionDragLast = null;
        this.render();
    }
    ...
    deleteSelectedStroke() {
        ...
        const index = this.strokes.indexOf(this.selectedStroke);
    ...
    renderSelectionOverlay(ctx) {
        if (this.currentTool !== 'select' || !this.selectedStroke) return;
        ...
        let maxY;
        if (this.selectedStroke.fillRect) {
            const rect = this.selectedStroke.fillRect;
            minX = rect.x;
            minY = rect.y;
            maxX = rect.x + rect.w;
            maxY = rect.y + rect.h;
        ...
        const scale = Math.max(0.2, this.view2D.scale || 1);
    ...
    startDrawing(e) {
        ...
        this._cancelStrokeRender();
        try {
            this._captureStrokeBase();
        } catch (error) {
            // Base capture is an optimization for live preview. A failure must
            // never prevent the stroke itself from being created.
            console.warn('[Animation2D] Stroke base capture skipped:', error);
        }
        ...
        const symmetryAxisX = this.symmetryXEnabled
            ? this.getSymmetryAxisX()
            : null;
        ...
        this.currentStroke = {
            ...
            // IMPORTANT:
            // freeze the symmetry axis at the moment this stroke is created.
            symmetryAxisX,

            points: [{
                x: pos.x,
                y: pos.y,
                p: this._pressureForEvent(e)
            }]
        };
    ...
    draw(e) {
        ...
        if (this.currentTool === 'pencil' || this.currentTool === 'eraser') {
            ...
            for (let i = 1; i <= steps; i++) {
                ...
                points.push({
                    ...
                    p: this._pressureForEvent(e, distance)
                });
            }
        ...
        // Input events can arrive much faster than the display refresh rate.
        this._scheduleStrokeRender();
    }
    ...
    stopDrawing(event) {
        ...
        if (this.currentStroke?.points?.length) {
            ...
            if (this.autoKeyEnabled !== false) {
                this.saveKeyframe({ exposure: this.drawingExposure, reason: 'stroke' });
            }
            this.currentStroke = null;
        }
    ...
    saveKeyframe(options = {}) {
        const beforeState = options.beforeState || (window.historyManager ? window.historyManager.capture2DState() : null);

        const frameIndex = this.getCurrentFrameIndex();
        this.keyframes.set(frameIndex, this._cloneStrokeData(this.strokes));

        const exposure = Number(options.exposure ?? this.drawingExposure ?? 1);
        this.setFrameExposure(frameIndex, exposure, false);
        this.syncWithTimeline();
        this.updateTimelineUI();
        this.syncTimelineMarkers();

        if (window.historyManager && !window.historyManager.isExecuting && beforeState) {
            const afterState = window.historyManager.capture2DState();
            if (afterState && JSON.stringify(beforeState.keyframes) !== JSON.stringify(afterState.keyframes)) {
                const reasonName = options.reason === 'delete' ? '2D Delete Keyframe' :
                    options.reason === 'clear' ? '2D Clear Frame' :
                        options.reason === 'layer' ? '2D Layer Edit' :
                            `2D Keyframe (Frame ${frameIndex})`;
                window.historyManager.addStep({
                    type: '2d_keyframe',
                    name: reasonName,
                    before: beforeState,
        ...
        window.dispatchEvent(new CustomEvent('animation2d:keyframe-recorded', {
            detail: {
                frame: frameIndex,
    ...
    commitKeyframe(options = {}) {
        return this.saveKeyframe({ ...options, reason: options.reason || 'timeline' });
    }
    deleteCurrentFrameKeyframe() {
        const beforeState = window.historyManager ? window.historyManager.capture2DState() : null;
        const frameIndex = this.getCurrentFrameIndex();
        this.keyframes.delete(frameIndex);
        for (const [frame, data] of this.keyframes.entries()) {
            if (data?._isHoldFrame && data._sourceFrame === frameIndex) this.keyframes.delete(frame);
        }
        this.strokes = this._cloneStrokeData(this._resolveFrameData(frameIndex));
        this.syncWithTimeline();
        this.updateTimelineUI();
        this.syncTimelineMarkers();
        this.render();

        if (beforeState && window.historyManager && !window.historyManager.isExecuting) {
            const afterState = window.historyManager.capture2DState();
            window.historyManager.addStep({
                type: '2d_keyframe',
                name: `2D Delete Keyframe (Frame ${frameIndex})`,
                before: beforeState,
                after: afterState
            });
        }
 ...
 */
    syncWithTimeline() {
        ...
        Object.keys(timelineMap).forEach((frame) => {
            if (timelineMap[frame]?.source === '2d_animation') delete timelineMap[frame];
        });
        ...
        this.keyframes.forEach((strokeData, frame) => {
            ...
            timelineMap[frame] = {
                ...
                exposure: this.drawingExposure
            };
        });

        window.keyframes.set(this.timelineObject.uuid, timelineMap);
        window.TimelineBinaryEvaluator?.invalidateCache?.(this.timelineObject.uuid);
     ...
     */
    setFrameExposure(frameIndex, exposureFrames = 1, sync = true) {
        const kfData = this.keyframes.get(frameIndex);
        if (!Array.isArray(kfData)) return;

        const normalized = Number.isFinite(exposureFrames)
            ? Math.max(1, Math.min(24, Math.round(exposureFrames)))
            : 24;
        this.drawingExposure = normalized;

        for (const [frame, data] of this.keyframes.entries()) {
            if (data?._isHoldFrame && data._sourceFrame === frameIndex) {
                this.keyframes.delete(frame);
            }
        }

        for (let offset = 1; offset < normalized; offset++) {
            const holdFrame = frameIndex + offset;
            if (Array.isArray(this.keyframes.get(holdFrame))) break;
            this.keyframes.set(holdFrame, {
                _isHoldFrame: true,
                _sourceFrame: frameIndex,
                _holdType: normalized === 1 ? '1s' : normalized === 2 ? '2s' : normalized === 3 ? '3s' : 'hold'
            });
        }
    ...
    onTimeUpdate() {
        ...
        this.strokes = this._cloneStrokeData(frameData).map((stroke) => {
            if (!stroke.layerId) stroke.layerId = 'layer_0';
            return stroke;
        });
        this.syncTimelineMarkers();
        this.render();
 ...
 */
    ...
    getSymmetryAxisX() {
        const center = this._inverseViewTransform(
    ...
    createMirroredStroke(stroke) {
        ...
        const axisX = Number.isFinite(stroke.symmetryAxisX)
            ? stroke.symmetryAxisX
            : this.getSymmetryAxisX();
    ...
    enterMode() {
        ...
        if (controls) {
            ...
            this.savedViewportState.controlsTarget = controls.target?.clone?.() || null;
        ...
        // Preserve the world once, then apply Blender-style white studio shading.
        this._captureViewportEnvironment();
        this._applyWhiteWorkspaceBackground();

        // Always preserve the live 3D camera so the underlay button can restore it.
        if (window.camera) {
            const cam = window.camera;
            this.savedViewportState.camPos = cam.position.clone?.() || null;
            this.savedViewportState.camRot = cam.rotation.clone?.() || null;
            this.savedViewportState.camUp = cam.up?.clone?.() || null;
    ...
    exitMode() {
        ...
        document.body.classList.remove('animation-2d-mode-active', 'animation-2d-3d-underlay');
        ...
        this._restoreViewportEnvironment();
        ...
        if (cam && this.savedViewportState.camFov !== undefined) {
            ...
            if (this.savedViewportState.camUp) cam.up.copy(this.savedViewportState.camUp);
            cam.fov = this.savedViewportState.camFov;
            cam.updateProjectionMatrix?.();
            if (controls?.target && this.savedViewportState.controlsTarget) {
                controls.target.copy(this.savedViewportState.controlsTarget);
                controls.update?.();
            }
        }
    ...
    loadReferenceImage(file) {
        ...
        reader.onload = () => {
            ...
            img.onerror = () => {
                console.warn('[2D] Failed to load reference image:', file.name);
            };
    ...
    drawReferenceImage(ctx) {
        ...
        ctx.globalAlpha = Math.max(
            0,
            Math.min(1, this.referenceImageOpacity)
        );
...
// Global instance
window.Animation2DManager = Animation2DManager;
