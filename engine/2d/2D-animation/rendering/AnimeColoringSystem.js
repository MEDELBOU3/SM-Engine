...
class AnimeColoringSystem {
    ...
    applyPostEffects() {
        ...
        if (!ctx || !canvas || canvas.width < 4 || canvas.height < 4) return;
    ...
    renderBloom(ctx, canvas) {
        ...
        const w = Math.floor(Number(canvas?.width) || 0);
        const h = Math.floor(Number(canvas?.height) || 0);
        if (!ctx || !canvas || w < 4 || h < 4) return false;

        // Create threshold buffer
        const bloomCnv = document.createElement('canvas');
        const bloomWidth = Math.max(1, Math.floor(w / 4));
        const bloomHeight = Math.max(1, Math.floor(h / 4));
        bloomCnv.width = bloomWidth; // Downscale for performance and blur quality
        bloomCnv.height = bloomHeight;
        const bCtx = bloomCnv.getContext('2d');
        if (!bCtx) return false;

        // Draw downscaled version
        bCtx.drawImage(canvas, 0, 0, bloomWidth, bloomHeight);
