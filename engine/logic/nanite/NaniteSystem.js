...
class NaniteSystem {
    ...
    addMesh(originalMesh, options = {}) {
        ...
        // NEVER process engine gizmos/helpers with Nanite
        if (this._isEngineInternalObject(originalMesh)) {
            return null;
        }
        ...
        if (
            ...
            !this.simplifyModifier
        ) {
            if (!this.simplifyModifier) {
                console.warn(
                    "NaniteSystem: SimplifyModifier not available, cannot add mesh:",
                    originalMesh.name
                );
            }
            return null;
    ...
    _isEngineInternalObject(obj) {
        if (!obj) return true;

        const ud = obj.userData || {};

        // Explicit engine/system markers
        if (
            ud.isSystemObject === true ||
            ud.isHelper === true ||
            ud.isInternal === true ||
            ud.ignoreInHierarchy === true ||
            ud.isNaniteLOD === true ||
            ud.isNaniteOriginal === true
        ) {
            return true;
        }

        // Three.js helpers / controls
        if (
            obj.isHelper ||
        ...
        // Parent-chain protection
        let parent = obj.parent;

        while (parent) {
            const pName = String(parent.name || '').toLowerCase();
            const pType = String(parent.type || '').toLowerCase();
            const pud = parent.userData || {};

            if (
                pud.isSystemObject === true ||
                pud.isHelper === true ||
                pud.isInternal === true ||
                pud.ignoreInHierarchy === true ||
                pName === 'transformcontrolsgizmo' ||
                pName === 'transformcontrols' ||
                pName === 'gizmoplanes' ||
                pName.includes('transformgizmo') ||
                pType === 'transformcontrolsgizmo' ||
                pType === 'transformcontrolsplane'
            ) {
                return true;
            }
